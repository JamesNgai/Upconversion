...
unsigned __int64 n, truensync = 0, t_index, t_index2, oflcorrection = 0, sync_0 = 0, sync_1 = 0, sync_2 = 0, timediff, q;
int result, j, k, g, r, dtime_0, dtime_1, index1, index2, x = 0, y = 0, sub_chan, count_in_window = 0,
diff[7], p[7],
count[6] = { 0 }, count_g2[200] = { 0 },// define integer arrays with intialised values of zero for each element
count_result[4][2000] = { 0 },
coincidencecounter[2080] = { 0 },
count_ch2[60000] = { 0 },
count_ch1[4][60000] = { 0 },
...
unsigned int buffer[TTREADMAX];
 ...
 int InputChannelOffset0 = 5800;
 int InputChannelOffset1 = 2100;
 ...
 */
 ...
 retcode = TH260_SetInputChannelOffset(dev[0], 0, InputChannelOffset0);
 if (retcode < 0)
 {
	 TH260_GetErrorString(Errorstring, retcode);
	 printf("\nTH260_SetInputChannelOffset error %d (%s). Aborted.\n", retcode, Errorstring);
 ...
 retcode = TH260_SetInputChannelOffset(dev[0], 1, InputChannelOffset1);
 ...
 printf("InputChannelOffsetCh1 : %ld\n", InputChannelOffset0);
 printf("InputChannelOffsetCh2 : %ld\n", InputChannelOffset1);
			...
			for (RecNum = 0; RecNum < nRecords; RecNum++) {
				...
				else //regular input channel
				{
					...
					if (MID(buffer[RecNum], 25, 31) == 0) // Channel 1: First channel
					{
						...
						index1 = floor(MID(buffer[RecNum], 10, 25));  // position in period with resolution bin according to our settings
						if(index1 >= (int)60 && index1 <= (int)220) {
							count_result[0][index1]++;
						...
						//double period = 1 / (double)Syncrate;
							...
							sync_1 = sync_0;
							count_ch1[0][t_index]++; // time traces for channel 1
							count_in_window = 1;
						}
					}

					if (MID(buffer[RecNum], 25, 31) == 1) // Channel 2: Second Input Channel
					{
						index2 = floor(MID(buffer[RecNum], 10, 25));
						if (index2 >= (int)60 && index2 <= (int)220) {
							
							sync_0 = oflcorrection + MID(buffer[RecNum], 0, 10);
							t_index = (sync_0*1e-6*1e-6*(52e3));
							count_ch1[1][t_index]++;// building time traces for channel 2 (ignore ch1 label for now)
							count_result[1][index2]++; //single counts histogram(int array)
					...
					{	
						...
						if (q < 10 && q == 0)
						{
							...
							coincidencecounter[timediff]= coincidencecounter[timediff]+1;
							max_ct++;
						}
					}
...
strcat(output_filename[0], "_histogram1.txt");
strcat(output_filename[1], "_unused.txt");
strcat(output_filename[2], "_coincidence.txt");
...
for (j = 0; j < nb_bins; j++) {
	fprintf(foutput[0], "%d     %d     %d   \n", j, count_result[0][j], count_result[1][j] );
}
 ...
 for (j = 0; j<nb_bins+1; j++)
 {
	 fprintf(foutput[2], "%d     %d     \n", j, coincidencecounter[j] );
 }
